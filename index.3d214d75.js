...
function getTargetZone(clientX, clientY) {
    ...
    if (isPointerInZone(grid.getBoundingClientRect())) return grid;
    if (isPointerInZone(free.getBoundingClientRect())) return free;
    return null;
}
...
function toggleGridScroll() {
    try {
        if (grid.scrollHeight > grid.clientHeight) grid.classList.add("grid_scroll");
        else grid.classList.remove("grid_scroll");
    } catch (e) {
        error.innerHTML = "Internal error, please contact with developers";
    }
}
function insertToZone(zone, elem) {
    ...
    leaveZone(zone);
    try {
        if (zone === grid) insertToGridZone(elem);
        if (zone === free) insertToFreeZone(elem);
    } catch (e) {
        error.innerHTML = "Internal error, please contact with developers";
    }
}
...
function startDrag(pageX, pageY, pointerType) {
    ...
    try {
        ...
        if (pointerType === "touch") element.addEventListener("pointerup", elementPointerUpHandler);
        else draggableElement.elem.addEventListener("pointerup", elementPointerUpHandler);
...
function elementPointerUpHandler(event) {
    ...
    if (!targetZone) draggableElement.elem.remove();
    else insertToZone(targetZone, draggableElement.elem);
    draggableElement = {};
}
...
},{}]},["awEvQ","bB7Pu"], "bB7Pu", "parcelRequired37e")
